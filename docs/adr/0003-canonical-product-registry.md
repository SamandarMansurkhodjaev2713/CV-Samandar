...
Расширенный текст главной остаётся в `src/content/content.js`, а содержание 15 кейсов — в `src/projects/landings-data.js` и `src/projects/landings-new.js`. Валидатор связывает эти слои и останавливает build, если нарушены уникальность, порядок, 24/9/15 split, route contract, локальный паритет, asset contract или public-safety boundary.
