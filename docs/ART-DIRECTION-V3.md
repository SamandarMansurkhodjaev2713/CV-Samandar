# Art direction V3 — Proof Laboratory

Статус: локальный release candidate в ветке
`codex/awwwards-art-direction-v3`; production release ещё не заявлен.
...
Signature scene — `Proof Chamber`: физический лабораторный стенд ведёт
необработанный графит через оптический QA-шлюз к законченному модулю. Поэтому
визуальный объект буквально показывает `BUILD → VERIFY → SHIP`, а не просто
создаёт абстрактную технологичную атмосферу. Его единственный responsive
`<picture>` доступен ещё до React как frame-zero LCP/fallback; React владеет
смысловым overlay, proof-rail и интерактивным слоем, но не дублирует медиа.
...
Короткие телефоны до 360×640 получают отдельную композицию высотой 88svh:
вся рейка, тезис, CTA и proof-rail остаются в первом экране, а Signal виден
минимум на 64 px. Landscape и короткий desktop также имеют собственные
...
|---:|---|---|---|
| 01 | Hero | physical proof instrument, split laboratory field | позиционирование и первое действие |
