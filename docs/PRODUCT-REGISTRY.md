...
- desktop-вступление намеренно показывает **первые 4 карточки** — DentForma,
  Klawis, TTYL Platform и BelfProctor, а кнопка «Показать ещё 26» раскрывает
  весь каталог без изменения порядка; число хранится как
  `PRODUCT_FEATURED_COUNT` рядом с canonical registry;
