...
- Desktop-cursor пересобран как verification instrument: в покое остаётся
  тихая точка, а над реальным действием появляются измерительная рамка,
  локализованный глагол и координаты текущей главы. Touch-профили курсор не
...
- V249 переводит главную из насыщенного pseudo-OS языка в цельную editorial
  release-систему: Hero оставляет один физический знак `RELEASE / ONE OWNER`,
  navbar — только идентичность, текущую главу, действие и меню, а кнопки
  получают спокойную округлую геометрию без clipped sci-fi декора.
- На mobile оглавление стало чистой двухколоночной картой сайта; закрытое меню
  гарантированно не остаётся поверх страницы. Нижний chapter dock упрощён до
  названия текущей секции и одного действия, без шкалы, координаты и повторной
  нумерации.
- Из длинных разделов убраны декоративные ghost-number, telemetry labels,
  scanlines и дублирующие схемы. Содержание, реальные технические факты,
  chapter markers и продуктовые изображения сохранены; межсекционные отступы
  увеличены для более спокойного чтения.
- Mobile Services больше не показывает пустую раскрытую область при входе по
  deep link: открытый ответ всегда заканчивается в читаемой финальной позе.
  Project still-life изображения выводятся без искусственного scanline-фильтра.
...
- About больше не имитирует профильный dashboard: live-clock, GitHub telemetry,
  ...
  `BRIEF → BUILD → VERIFY → RELEASE`, проверяемыми фактами и текущим фокусом,
  полностью локализованными на RU/EN/UZ.
- Главная больше не загружает `gh.js` и не делает runtime-запросы к GitHub API
  ради декоративного профиля. About имеет полную статичную финальную позу,
  поэтому не меняет высоту после загрузки и не зависит от rate limit или сети.
...
- Projects получил более читаемый museum pass: уникальный продуктовый accent,
  световую позицию и crop каждой карточки, устойчивую асимметричную
  desktop-сетку `7/5`, полный mobile filmstrip с category-фильтрами и приоритет
  задачи/результата вместо повторяющейся служебной подписи.
- Первичный React mount разделён на пять коротких смысловых стадий. Hero и
  Signal доступны первым commit, остальные главы добавляются через
...
- Ранний клик по главе в fullscreen Index больше не теряется во время
  ...
  intent; более новое намерение отменяет ожидающее. Зафиксированный cloud
  retry подтверждён и закрыт стресс-прогоном 20/20 без retry.
- Секционные reveal-маски получили безопасный верхний внутренний отступ, а
  About — три управляемые смысловые строки с отдельной mobile-типографикой.
  Кириллические диакритики больше не обрезаются, заголовок не разъезжается на
  320–1920 px и выдерживает 200% text zoom.
- Responsive matrix теперь проверяет геометрию About на одиннадцати canonical
  viewport: границы каждой title line, внутренний overflow, четыре ownership
  stage, четыре факта, минимальный кегль и сохранение трёхчастного reading order.
