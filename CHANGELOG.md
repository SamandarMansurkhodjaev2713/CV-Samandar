...
- V6 `Proof Compiler` заменяет статичную предметную Hero-фотографию на
  code-native причинно-следственную сцену `INPUT → BUILD → QUALITY GATE →
  RELEASE`. Один мотив непрерывно связывает frame zero, Intro, Hero и preview
  полноэкранного Index без растрового media gate на критическом пути.
- Контекстный desktop-cursor получил измерительные jaws, target frame,
  координаты активной главы и глагол действия; touch-профили по-прежнему не
  имитируют курсор.
...
- Верхняя навигация стала полноширинной инструментальной рейкой с текущей
  главой, прогрессом, языком и одним главным действием. Полноэкранный Index
  показывает 12 реальных глав в 3×4, 2×6 или 4×3 композиции по доступной
  геометрии, не обрезая локализованные подписи.
- Основной текст переведён на self-hosted Inter с Cyrillic/Latin/Latin Extended;
  Oswald и Cormorant сохранены как характерный display/editorial слой, а не
  используются для длинного чтения.
