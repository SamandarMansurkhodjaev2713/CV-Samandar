...
// ─────────────────────────────────────────────────────────────────────────────
// TRUST — QUALITY LIFECYCLE
//
// Quality is shown as a release decision that matures across three phases,
// rather than as a detached checklist or a decorative paper document. The six
// existing, factual controls stay intact and are grouped by when they protect
// the product: before code, during the build and before release.
// ─────────────────────────────────────────────────────────────────────────────
function Trust({ t }) {
  const ref = useRevealRoot([t]);
  ...
  return (
    <section data-section="trust" id="trust" data-enter="slide-right" ref={ref}>
      <div className="shell">
        ...
        <div className="quality-system" data-reveal>
          <ol className="quality-stages">
            {stages.map(function renderStage(stage, stageIndex) {
              const stageProof = (proofOrder[stageIndex] || []).map(function pickProof(index) { return proof[index]; }).filter(Boolean);
              return (
                <li className="quality-stage" key={stage.k} data-reveal data-reveal-delay={(stageIndex * 0.08).toFixed(2)}>
                  <header className="quality-stage-head">
                    <span className="quality-stage-number" aria-hidden="true">{String(stageIndex + 1).padStart(2, "0")}</span>
                    <div>
                      <h3>{stage.k}</h3>
                      <p>{stage.v}</p>
                    </div>
                  ...
                  <ul className="quality-checks">
                    {stageProof.map(function renderCheck(item) {
                      return (
                        <li className="quality-check" key={item.k}>
                          <span className="quality-check-mark" aria-hidden="true">✓</span>
                          <div>
          ...
          <div className="quality-decision">
            <span className="quality-decision-label mono">{t.trust.decision_label}</span>
            <strong>{t.trust.decision}</strong>
          </div>
        </div>
      </div>
    </section>
